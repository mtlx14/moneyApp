  ...
  return (
    <CollapsibleRow open={included}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, overflow: 'hidden' }}>
        <Text style={{ color: theme.text._3, fontSize: fS.homeSubText, fontWeight: theme.fw.home_acc_text }}>{`${bill.emoji}   ${bill.label} ${bill.inMonths > 0 && bill.firstMonth ? currentInstallment(bill.firstMonth, monthOffset) + 1 + '/' + bill.inMonths : ''}`}</Text>

        <AnimatedSwapTextS type={'debt'} color={theme.text.strong} value={amountForMonth(bill, monthOffset + 1)} />
      </View>
    </CollapsibleRow>
  );
}
