...
export default function UserTag({}) {
  ...
  const themeItemSize = (windowWidth * 0.6 - 15 * (themeItemsForRow + 1)) / themeItemsForRow;

  const renderThemeItem = (t) => (
    <Pressable
      onPress={() => {
        updateTheme(t.name);
        setModal_1_exitRight(false);
    ...
    >
      <LinearGradient colors={[t.bg.primary_2, t.bg.primary]} start={{ x: 0.2, y: 1 }} end={{ x: 0, y: 0 }} style={{ width: themeItemSize, height: themeItemSize, borderRadius: 10 }} locations={[0, 1]}></LinearGradient>
    </Pressable>
  );
