  ...
  return (
    <CollapsibleRow open={included}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, overflow: 'hidden' }}>
        <Text style={{ color: theme.text._3, fontSize: fS.homeSubText }}>{`${bill.emoji}   ${bill.label} ${bill.inMonths > 0 && bill.firstMonth ? currentInstallment(bill.firstMonth, monthOffset) + 1 + '/' + bill.inMonths : ''}`}</Text>
