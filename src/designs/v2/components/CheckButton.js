...
export default function CheckButton({ bill }) {
  ...
  return (
    <Pressable onPress={() => handleOnPress()} style={{ height: windowWidth * 0.1 - 10, aspectRatio: 1 / 1, backgroundColor: checked ? (yellow ? theme.bg.yellow : theme.text.green) : theme.bg.tr_1, borderRadius: 5, justifyContent: 'center', alignItems: 'center' }}>
      {checked && (
